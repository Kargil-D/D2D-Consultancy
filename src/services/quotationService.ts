...
const QUOTATION_SUMMARY_SELECT = {
  ...
  createdDate: true,
  updatedDate: true,
  lead: { select: { id: true, seq: true, customerName: true, mobile: true } },
 ...
 * quotation's Lead's own seq) — since every Quotation/Booking tied to that Lead shares the
 * same code, this alone finds every quotation belonging to that customer's journey. */
...
export async function buildPublicQuoteData(quotation: NonNullable<Awaited<ReturnType<typeof getQuotation>>>) {
  ...
  return {
    quoteCode: trackingCode(quotation.lead.seq),
    ...
    heroImage,
    departureDate: quotation.departureDate ? quotation.departureDate.toLocaleDateString("en-IN") : null,
