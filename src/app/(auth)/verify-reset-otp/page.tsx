...
import { verifyResetOtpApi, resendOtpApi } from "@/services/authService";
...
function VerifyResetOtpForm() {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    setSubmitting(true);
    try {
      const { resetTicket } = await verifyResetOtpApi(email, otp);
      window.sessionStorage.setItem(RESET_TICKET_KEY, resetTicket);
  ...
  const handleResend = async () => {
    ...
    setResending(true);
    try {
      await resendOtpApi(email, "ForgotPassword");
      setInfo("If an account matches, a new code has been sent.");
      restart();
    } catch (err) {
  ...
  return (
    <div>
      ...
      <p className="mt-1.5 text-sm text-slate-500 mb-7">
        If an account exists for <strong>{email || "that email"}</strong>, we&apos;ve sent a 6-digit
        code.
      </p>
