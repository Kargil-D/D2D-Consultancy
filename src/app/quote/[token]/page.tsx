...
export default async function PublicQuotePage({ params }: PageProps) {
  ...
  const hotelGroups = data.hotelOptions.filter((g) => g.hotels.length > 0);
                  ...
                  {hotelGroups.map((group) => (
                    <div key={group.id}>
                      {hotelGroups.length > 1 && (
                        <div className="inline-flex items-center px-3 py-1 rounded-full bg-blue-600 text-white text-xs font-bold tracking-wide mb-3">
                          {group.label}
                        </div>
                      )}
