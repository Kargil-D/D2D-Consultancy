...
export default function QuotationActivitiesEditor({ activities, onChange }: QuotationActivitiesEditorProps) {
  ...
  const addImage = (idx: number, url: string) => {
    if (!url) return;
    update(idx, { images: [...(activities[idx].images ?? []), url] });
  };
  const removeImage = (idx: number, imgIdx: number) => {
    update(idx, { images: (activities[idx].images ?? []).filter((_, j) => j !== imgIdx) });
  };

  return (
    <div className="space-y-4">
      <div className="flex justify-between items-center">
        <p className="text-xs text-slate-500">e.g. Safari World, Coral Island, Sky Walk, Theme Park.</p>
