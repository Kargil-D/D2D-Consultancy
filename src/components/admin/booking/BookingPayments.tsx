...
interface Props {
  ...
  onAddCustomerPayment: (payload: { paymentDate: string; paymentMode: PaymentMode; amount: number; transactionReference?: string; referenceImageUrl?: string; remarks?: string }) => Promise<void>;
  /** Quotation's grand total (cost + margin + GST) — undefined when no Quotation is linked. */
  dealPrice?: number;
}
...
export default function BookingPayments({ customerPayments, onAddCustomerPayment, dealPrice }: Props) {
  ...
  const balance = dealPrice !== undefined ? dealPrice - totalPaid : undefined;

  const addCustomer = async () => {
    if (!cDate || cAmount <= 0) return;
    setSavingC(true);
    try {
      await onAddCustomerPayment({ paymentDate: cDate, paymentMode: cMode, amount: cAmount, transactionReference: cRef, referenceImageUrl: cRefImage || undefined, remarks: cRemarks });
      setCDate(""); setCAmount(0); setCRef(""); setCRefImage(""); setCRemarks("");
    } finally {
      setSavingC(false);
    }
  };
    ...
    <div>
        ...
        <table className="w-full text-sm">
          <thead>
            <tr className="text-left text-xs font-semibold text-slate-500 uppercase border-b border-slate-100">
              <th className="px-3 py-2">Date</th><th className="px-3 py-2">Mode</th><th className="px-3 py-2 text-right">Amount</th><th className="px-3 py-2">Reference</th><th className="px-3 py-2">Ref. Image</th><th className="px-3 py-2">Remarks</th>
            </tr>
          </thead>
          <tbody>
      ...
      <div className="grid grid-cols-1 sm:grid-cols-3 gap-3 mt-3">
        <div className="rounded-xl border border-slate-200 bg-slate-50 p-3">
          ...
          <div className="text-lg font-bold text-slate-900 mt-0.5">{dealPrice !== undefined ? formatINR(dealPrice) : "—"}</div>
        </div>
        <div className="rounded-xl border border-slate-200 bg-slate-50 p-3">
